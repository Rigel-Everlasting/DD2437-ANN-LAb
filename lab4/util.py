...
def sigmoid(support):
    mask = (support > 0)
    positive_out = np.zeros_like(support, dtype='float64')
    negative_out = np.zeros_like(support, dtype='float64')

    positive_out = 1 / (1 + np.exp(-support, positive_out, where=mask))
    positive_out[~mask] = 0

    expZ = np.exp(support, negative_out, where=~mask)
